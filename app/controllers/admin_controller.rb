class AdminController < ApplicationController
  ...
  def index
    @orders = Order.where(fullfillment: false).order(created_at: :desc).take(5)
    @quick_stats = {
      sales: Order.where(created_at: Time.now.midnight..Time.now).count,
      revenue: Order.where(created_at: Time.now.midnight..Time.now).sum(:grand_price).round(),
      avg_sale: Order.where(created_at: Time.now.midnight..Time.now).average(:grand_price),
    ...
    @revenue_by_day = @orders_by_day.map { |day,orders| [day.strftime("%A"), orders.sum(&:grand_price)] }
