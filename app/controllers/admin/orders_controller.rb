class Admin::OrdersController < AdminController
  ...
  def index
    @fullfilled_orders = Order.where(fullfillment: true).order(created_at: :desc)
    @unfullfilled_orders = Order.where(fullfillment: false).order(created_at: :desc)
  end
  ...
  private
    ...
    def admin_order_params
      params.require(:order).permit(:customer_email, :fullfillment, :address, :grand_price)
    end
end
